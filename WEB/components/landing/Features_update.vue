...
async function updateData() {
  try {
    const response = await axios.put(`http://localhost:5000/A1_and_A2_years/update/id/${id.value}`, {
      發生年度: year.value,
      發生月: month.value,
      區序: region.value,
...
<template>
  <div class="mt-16 md:mt-0">
    <h2 class="text-4xl lg:text-5xl font-bold lg:tracking-tight">更新資料</h2>
    <p class="text-lg mt-4 text-slate-600">請透過下方文字方塊及選擇按鈕得以更新一筆資料進入系統儲存</p>
    <div class="mt-4 flex space-x-4">
      <input v-model="id" class="p-2 border rounded" type="text" placeholder="特定資料ID" />
      ...
      <!-- 確定按鈕，點擊後調用updateData函數 -->
      <button class="p-2 bg-blue-500 text-white rounded" @click="updateData">確定</button>
    </div>

    <!-- 顯示API更新資料的結果 -->
    <div v-if="apiResponse" class="mt-8">
      <h2 class="text-2xl font-bold mb-4">更新資料結果</h2>
